...
std::string body_to_python(const Body& body, const std::string& ident) noexcept
{
    std::string result = "";

    for (Statement* statement : body)
    {
        result += statement->to_python(ident) + "\n";
    }
...
std::string param_list_to_python(const ParamList& param_list, const std::string& ident) noexcept
{
    if (param_list.empty())
    {
        return "";
    ...
    std::string result = it->first + " : " + it->second->to_python(ident);

    ++it;

    for ( ; it != param_list.end(); ++it)
    {
        result += ", " + it->first + " : " + it->second->to_python(ident);
    }
